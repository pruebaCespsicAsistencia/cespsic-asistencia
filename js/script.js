...
function initializeGoogleSignIn() {
    try {
        google.accounts.id.initialize({
            ...
            // ✅ ELIMINADO: ux_mode (causa problemas de CORS)
            // ✅ ELIMINADO: use_fedcm_for_prompt
        });
...
function showGoogleButtonInModal() {
    ...
    // ✅ RENDERIZAR BOTÓN CON CONFIGURACIÓN SIMPLIFICADA
    setTimeout(() => {
        const buttonDiv = document.getElementById('google-signin-button');
        if (buttonDiv) {
            try {
                // ✅ Reinicializar ANTES de renderizar
                google.accounts.id.initialize({
                    client_id: GOOGLE_CLIENT_ID,
                    callback: handleCredentialResponse,
                    auto_select: false,
                    cancel_on_tap_outside: false
                    // ✅ NO incluir ux_mode aquí
                });
                ...
                // ✅ FALLBACK: Botón HTML personalizado
                buttonDiv.innerHTML = `
